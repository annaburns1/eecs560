...
class MyBST
{
  private:
    ...
    void balance(BinaryNode*& t)
    {
        ...
        if(int(height(t->left)) - int(height(t->right)) > 1) {
        ...
        else if(int(height(t->right)) - int(height(t->left)) > 1) {
    ...
    void rotateLeft(BinaryNode*& t)
    {
        ...
        k1->right = t;
        t->height = std::max(height(t->left), height(t->right)) +1;
        k1->height = std::max(height(k1->left), height(t->right)) +1;
        t = k1;
        // code ends
    }
    ...
    void rotateRight(BinaryNode*& t)
    {
        ...
        k1->height = std::max(height(k1->right), height(t->left)) +1;
        t = k1;
        // code ends
    }
  ...
  public:
    ...
    {
        // code begins
// return height of find max or find min?
      // size_t depthMin = height(findMin(root));
      // size_t depthMax = height(findMax(root));
      // return(std::max(depthMin, depthMax));
      // size_t num = 0;
      // BinaryNode* tempR;
      // BinaryNode* tempL;
      // if (root == nullptr) {
    ...
    bool lowestCommonAncestor(const ComparableType& x, const ComparableType& y, ComparableType& lca)
    {
        ...
        if(!(contains(x)) || !(contains(y))) {
          ...
          while(temp != nullptr) {
            if(x < temp->element && y < temp->element) {
              temp = temp->left;
            }
            else if((x > temp->element) && (y > temp->element)) {
              temp = temp->right;
            }
            else {
              break;
